...
void fitlambda(int file = 0, int trig = 0, int pt = 0, int frame = 0){
	...
	TFile* inputfile = new TFile(Form("~/polresults/20160707/MLEmethod/combinedrootfiles/sys_%d/lambda_trg%d_pt%d_frame%d.root",file,trig,pt,frame),"update");

	mle = (TH2F*)inputfile->Get(Form("mle_%d_%d_%d",trig,pt,frame));

	if(mle==0x0 || mle->GetMean()!=mle->GetMean() || mle->GetMean()==0 ){
	...
	//	double contour = minimum + 10;	
//	double deltacontour = 150.;
	double deltacontour = 5.;
	double contour = minimum + deltacontour;	
	...
	while(1){
		...
		if(xx<xmin || xx>xmax || yy<ymin || yy>ymax){//minimum point doesn't locate in the contour
	...
	gc2->SetTitle(Form("contour of minL+%.0f",deltacontour));
	gc2->SetLineWidth(1);
	...
	/*
	   ...
	   double likelihood_x = mle->GetXaxis()->FindBin(xmin+0.02*(i-1));
	   double likelihood_y = mle->GetYaxis()->FindBin(ymin+0.02*(j-1));
	   double bincontent = mle->GetBinContent(likelihood_x,likelihood_y);
	...
	TH2F* smallmle = new TH2F("smallmle","smallmle;#lambda_{#theta};#lambda_{#phi}",xxbin,xmin,xmax,yybin,ymin,ymax);
	TH2F* smallfit = new TH2F("smallfit","smallfit;#lambda_{#theta};#lambda_{#phi}",xxbin,xmin,xmax,yybin,ymin,ymax);
	for(int i=1;i<=xxbin;i++){
		for(int j=1;j<=yybin;j++){
			double mle_x = difference->GetXaxis()->GetBinCenter(i);
			double mle_y = difference->GetYaxis()->GetBinCenter(j);
			double difference_xy = mle->GetBinContent(mle->GetXaxis()->FindBin(mle_x),mle->GetYaxis()->FindBin(mle_y))-contfcn->Eval(mle_x,mle_y);
			difference->SetBinContent(i,j,difference_xy);
			smallmle->SetBinContent(i,j,mle->GetBinContent(mle->GetXaxis()->FindBin(mle_x),mle->GetYaxis()->FindBin(mle_y)));
			smallfit->SetBinContent(i,j,contfcn->Eval(mle_x,mle_y));
		}
	}		
	...
	smallmle->Write("",TObject::kOverwrite);
	smallfit->Write("",TObject::kOverwrite);
	/*
	   canvas[file][trig][pt][frame]->cd(4);
	...
	if(contfcn!=NULL)canvas[file][trig][pt][frame]->SaveAs(Form("~/polresults/20160707/MLEmethod/pdf/sys_%d/mle_%d_%d_%d_%d.pdf",file,file,trig,pt,frame));
	inputfile->Close();	
}
