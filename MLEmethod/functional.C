...
void functional(int file,int trig,int pt,int frame){

	rawMCdata(file,trig,pt,frame);
//	rawdata2D[file][trig][pt][frame];

	TF2* likelihood = new TF2("likelihood",fcn(double ltheta,double lphi),-1,1,-1,1,0,2);
...
Double_t fcn(Double_t ltheta,Double_t lphi){
fcn=0.;
TF2* sigma = new TF2*("sigma","1+[0]*[0]*x+(1-[0]*[0])*cos(2*[1])*y",-1,1,-1,1);

int nbinxeff = eff2D[file][trig][pt][frame][2]->GetNbinsX();
int nbinyeff = eff2D[file][trig][pt][frame][2]->GetNbinsY();

double costheta,phi,eff=0.;
int Npositron;
for(int i=1;i<=nbinxeff;i++){
	for(int j=1;j<=nbinyeff;j++){
		costheta = eff2D[file][trig][pt][frame][2]->GetXaxis()->GetBinCenter(i);
		phi = eff2D[file][trig][pt][frame][2]->GetYaxis()->GetBinCenter(j);
		sigma->SetParameter(0,costheta);
		sigma->SetParameter(1,phi);
		eff = eff2D[file][trig][pt][frame][2]->GetBinContent(i,j);
		Npositron = rawdata2D[file][trig][pt][frame]->GetBinContent(i,j);
		fcn += -1*Npositron*TMath::Log(sigma*eff);
	}
}
return fcn;
