...
TH2F* bestfit;
TH2F* check1Dfit;
TH2F* lambdathetapos1fit;
TH2F* lambdathetaneg1fit;
TH2F* lambdaphipos1fit;
TH2F* lambdaphineg1fit;
	...
	for(int trig=0;trig<3;trig++){
		for(int frame=0;frame<2;frame++){
			...
			for(int pt=0;pt<6;pt++){
				...
				c1->Divide(4,4);
				combinedfile = new TFile(Form("~/polresults/20160707/MLEmethod/combinedrootfiles/sys_%d/lambda_trg%d_pt%d_frame%d.root",file,trig,pt,frame),"read");		
				rootfile = new TFile(Form("~/polresults/20160707/MLEmethod/rootfiles/sys_%d/likelihood_%d_%d_%d_%d_1_1.root",file,file,trig,pt,frame),"read");
				chi2 = (TH2F*)combinedfile->Get(Form("mle_%d_%d_%d",trig,pt,frame));
				int xx,yy,zz;
				...
				lambda = (TGraphErrors*)combinedfile->Get("lambda");
				if(lambda==0x0) continue;
				cout<<"lambda = "<<lambda<<endl;
				lambda->GetPoint(0,lambdatheta,lambdaphi);
				...
				truth = (TH2F*)templates->Get(Form("theta_%d_phi_%d",chi2->GetXaxis()->FindBin(lambdatheta)-1,chi2->GetYaxis()->FindBin(lambdaphi)-1));
				truth->SetName(Form("template_%d_%d_%d_%d_theta_%d_phi_%d",file,trig,pt,frame,xx-1,yy-1));
				if(rebin==1){
					truth->RebinX(4);			
					truth->RebinY(4);
				}
				...
				if(truth!=0x0){
					bestfit = (TH2F*)truth->Clone();
					bestfit->SetName(Form("bestfite_%d_%d_%d_%d_theta_%d_phi_%d",file,trig,pt,frame,xx-1,yy-1));
	//				if(rebin==1){
	//					bestfit->RebinX(4);			
	//					bestfit->RebinY(4);
	//				}
					bestfit->Multiply(eff);
				}
				check1Dfit = (TH2F*)templates->Get(Form("theta_%d_phi_%d",chi2->GetXaxis()->FindBin(lambdatheta)-1,chi2->GetYaxis()->FindBin(lambdaphi)-1));
				check1Dfit->SetName(Form("check1Dfit_%d_%d_%d_%d_theta_%d_phi_%d",file,trig,pt,frame,xx-1,yy-1));
...
//					eff1D[file][trig][pt][frame][0]->Draw();	
				...
				lambdathetaneg1fit = (TH2F*)templates->Get(Form("theta_0_phi_%d",chi2->GetYaxis()->FindBin(lambdaphi)-1));
				lambdathetaneg1fit->SetName(Form("lambdathetaneg1fit_%d_%d_%d_%d_theta_%d_phi_%d",file,trig,pt,frame,xx-1,yy-1));
				if(lambdathetaneg1fit!=0x0){
					if(rebin==1){
						lambdathetaneg1fit->RebinX(4);
						lambdathetaneg1fit->RebinY(4);
					}	
					lambdathetaneg1fit->Multiply(eff2D[file][trig][pt][frame][2]);	
				}
				lambdathetapos1fit = (TH2F*)templates->Get(Form("theta_99_phi_%d",chi2->GetYaxis()->FindBin(lambdaphi)-1));
				lambdathetapos1fit->SetName(Form("lambdathetapos1fit_%d_%d_%d_%d_theta_%d_phi_%d",file,trig,pt,frame,xx-1,yy-1));
				if(lambdathetapos1fit!=0x0){
					if(rebin==1){
						lambdathetapos1fit->RebinX(4);
						lambdathetapos1fit->RebinY(4);
					}
					lambdathetapos1fit->Multiply(eff2D[file][trig][pt][frame][2]);
				}

				lambdaphineg1fit = (TH2F*)templates->Get(Form("theta_%d_phi_0",chi2->GetXaxis()->FindBin(lambdatheta)-1));
				lambdaphineg1fit->SetName(Form("lambdaphineg1fit_%d_%d_%d_%d_theta_%d_phi_%d",file,trig,pt,frame,xx-1,yy-1));
				if(lambdaphineg1fit!=0x0){
					if(rebin==1){
						lambdaphineg1fit->RebinX(4);
						lambdaphineg1fit->RebinY(4);
					}	
					lambdaphineg1fit->Multiply(eff2D[file][trig][pt][frame][2]);	
				}
				lambdaphipos1fit = (TH2F*)templates->Get(Form("theta_%d_phi_99",chi2->GetYaxis()->FindBin(lambdatheta)-1));
				lambdaphipos1fit->SetName(Form("lambdaphipos1fit_%d_%d_%d_%d_theta_%d_phi_%d",file,trig,pt,frame,xx-1,yy-1));
				if(lambdaphipos1fit!=0x0){
					if(rebin==1){
						lambdaphipos1fit->RebinX(4);
						lambdaphipos1fit->RebinY(4);
					}
					lambdaphipos1fit->Multiply(eff2D[file][trig][pt][frame][2]);
				}
				...
				if(bestfit!=0x0) bestfit->Scale(rawdata->Integral()/bestfit->Integral());
				if(check1Dfit!=0x0) check1Dfit->Scale(rawdata->Integral()/check1Dfit->Integral());
				if(lambdathetapos1fit!=0x0) lambdathetapos1fit->Scale(rawdata->Integral()/lambdathetapos1fit->Integral());
				if(lambdathetaneg1fit!=0x0) lambdathetaneg1fit->Scale(rawdata->Integral()/lambdathetaneg1fit->Integral());
				if(lambdaphineg1fit!=0x0) lambdaphineg1fit->Scale(rawdata->Integral()/lambdaphineg1fit->Integral());
				if(lambdaphipos1fit!=0x0) lambdaphipos1fit->Scale(rawdata->Integral()/lambdaphipos1fit->Integral());
				...
				rawdata_py->SetTitle("best fit of cos#theta");
				rawdata_py->Draw("");
				...
				lambdathetaneg1fit->ProjectionY("lambdathetaneg1fit_py");
				lambdathetaneg1fit_py->Draw("L same");
				lambdathetapos1fit->ProjectionY("lambdathetapos1fit_py");
				lambdathetapos1fit_py->Draw("L same");
				if(check1Dfit!=0x0){
					check1Dfit->ProjectionY("check1Dfit_py");
//					check1Dfit_py->Draw("same");
...
//						leg->AddEntry(template_theta_eff_py,"#lambda_{#phi} = 0","lpe");
					...
					rawdata_px->SetTitle("best fit of #phi");
					rawdata_px->Draw("");
					...
					lambdaphipos1fit->ProjectionX("lambdaphipos1fit_px");
					lambdaphipos1fit_px->Draw("same");
					lambdaphineg1fit->ProjectionX("lambdaphineg1fit_px");
					lambdaphineg1fit_px->Draw("same");

					if(check1Dfit!=0x0){
						check1Dfit->ProjectionX("check1Dfit_px");
//						check1Dfit_px->Draw("same");
...
//						leg->AddEntry(template_phi_eff_px,"#lambda_{#theta} = 0","lpe");	
					...
					c1->SaveAs(Form("~/polresults/20160707/MLEmethod/pdf/sys_%d/trig%d_pt%d_frame%d.pdf",file,trig,pt,frame));
					...
					c2->SaveAs(Form("~/polresults/20160707/MLEmethod/pdf/sys_%d/fitlambda_%d_%d_%d_%d.pdf",file,file,trig,pt,frame));
				}
			}
		}
	}
