...
void parameters(int ifile = 0,int itrig = 0, int xsect = 11, int ysect = 11){
	histograms = new TFile("histograms20161004.root","read");
	histograms_copy = new TFile("templates.root","read");
	correcteddata(ifile);
...
//	rawdata->Scale(1./rawdata->Integral());
	...
	eff->SetName(Form("eff_2dhist_%d_%d_%d_%d",ifile,itrig,ipt,iframe));
...
void correcteddata(int file=0){
	...
	for(;file<selectedfile;file++){
		...
		for(int trig=0;trig<NTRIG;trig++){
			if((file>=20 && file<=23) || (file>=29 && file<=30) || file==35) rawdatafile[file][trig] = new TFile(Form("rootfile/%s_trg%d_sys%d.ana.root",trigSet[trig].Data(),trig+3,file),"read");//marker
			else rawdatafile[file][trig] = new TFile(Form("rootfile/%s_trg%d_sys%d.ana.root",trigSet[trig].Data(),trig+3,0),"read");//marker
...
//					canvas[trig][pt][frame]->cd(4);
					...
					rawdata2D[file][trig][pt][frame]->SetName(Form("rawdata_%s_pT%d_frame%d",trigName[trig].Data(),pt,frame));
